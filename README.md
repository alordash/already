...
1. Add `hotcode` to `Cargo.toml` dependencies:
...
1. Add `hotcode` with `runtime` feature to `Cargo.toml` dependencies and build dependencies:

```toml
[dependencies]
hotcode = { version = "*", features = ["runtime"] }
...
3. Add `hotcode::runtime::build()` to your build script (the `build.rs` file in the root of your crate). This
   function
...
// build.rs
fn main() {
    hotcode::runtime::build();
}
```

4. Add `hotcode::runtime::add_runtime!();` anywhere in your crate's root (presumably in `lib.rs` file, if you're not
   sure see [examples](examples)).

5. Add `hotcode::runtime::start_watchers!(your_crate_name)` somewhere in your binary's `main` function. This spawns
   watcher
...
// bin.rs
fn main() {
    hotcode::runtime::start_watchers!(your_crate_name);
    // your code
}
```
...
- [ ] write about `hotcode::runtime::build` for tests and separate directories
